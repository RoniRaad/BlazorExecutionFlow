...
window.DrawflowBlazor = (function () {
    ...
    function autoSizeNode(elementId, nodeId, inLabels = [], outLabels = [], labelGap = 8)
    {
        ...
        const contentNode = nodeEl.querySelector('.drawflow_content_node .main-content');
        const title = nodeEl.querySelector(".title").innerText;
        let titleWidth = _measureTextPx(title, "1rem 'Helvetica Neue', Helvetica, Arial, sans-serif");
        const width = Math.max(contentNode.clientWidth, titleWidth);

        nodeEl.style.setProperty('--df-body-width', width + 'px');
    }
